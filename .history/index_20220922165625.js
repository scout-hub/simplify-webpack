 ...
 * @LastEditors: Zhouqi
 * @LastEditTime: 2022-09-22 16:55:31
 */
import fs from 'fs';
...
const resolveInputDep = (filePath) => {
    ...
    traverse.default(ast, {
        ImportDeclaration({
            ...
            // 处理文件的路径
            console.log(path.resolve('./src', node.source.value));
            const p = path.relative(process.cwd(), path.resolve(__filename, node.source.value))
            node.source.value = p;
        },
    });
